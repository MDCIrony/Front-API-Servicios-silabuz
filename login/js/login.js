...
formLogin.onsubmit = async function (event) {
  ...
  try {
    ...
    const response = await fetch('https://api-pagos-servicios.onrender.com/token/', options);
    ...
    if (data.detail) {
      ...
      const responseID = await fetch(`https://api-pagos-servicios.onrender.com/users/${body.username}/`);
